<template>
    <v-card>
        <v-card-title>By category <span v-if="isAdmin()">for suppliers</span></v-card-title>
...
export default {
    name: "CategoriesPercentageSelectedForMerchant",
    components: {PieChart},
    data() {
        return {
            options: {
    ...
    mounted() {
        ...
        api.getCategoriesPercentageSelectedForMerchant().then((resp) => {
            self.chartdata = {
