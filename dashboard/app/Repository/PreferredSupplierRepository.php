...
class PreferredSupplierRepository
{
    ...
    public function find(SearchParamsDto $searchParamsDto, int $userId): LengthAwarePaginator
    {
        ...
        $query = $user->preferredSuppliers()
            ->select(
                'u.id',
                'u.first_name',
                'u.username'
            )
            ->leftJoin('users as u', 'u.id', '=', 'users_preferred_suppliers.supplier_id')
            ->orderBy(
    ...
    {
        $supplier = $user->preferredSuppliers()->wherePivot('supplier_id', '=', $supplierId)->first();
        if ($supplier) {
            return false;
        }

        $user->preferredSuppliers()->attach($supplierId);
    ...
    public function delete(User $user, int $supplierId): bool
    {
        ...
        $user->preferredSuppliers()->detach($supplierId);
    ...
    public function getOptions($search = null): Collection
    {
        $query = DB::table('users')->distinct()
            ...
            ->where('roles.slug', '=', Role::ROLE_COMPANY_SLUG);
