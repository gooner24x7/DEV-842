...
The package uses the AITOOLV3 allocation approach: deterministic row-level rules run first, and only genuinely ambiguous `REVIEW` records are sent to a protected OpenAI Responses API fallback. The public allocation result is a row-level allocation hierarchy matching the HTML review tree.
...
- Groups validated row decisions into the template's allocation hierarchy.
...
```bash
cd /path/to/laravel-boq-allocator
export OPENAI_API_KEY="your-api-key"
export OPENAI_MODEL="gpt-5.6-luna"
```

The API key is only needed when the decision cache does not already contain a required review decision. Never commit the key or place it in a report directory.
...
The same allocator command for all three profiles is:

```bash
php bin/allocate.php --boq="/path/to/BoQ.xlsx" --template="templates/NRM1 template.csv" --output="output/nrm1-allocation.json"
php bin/allocate.php --boq="/path/to/BoQ.xlsx" --template="templates/NRM2 template.csv" --output="output/nrm2-allocation.json"
php bin/allocate.php --boq="/path/to/BoQ.xlsx" --template="templates/WD template.csv"   --output="output/wd-allocation.json"
```

Each command writes the existing JSON allocation structure to the specified output path. Unless `BOQ_DECISION_CACHE_PATH` is set, the command reads and updates `decision-cache.php` in the output directory.
...
Existing callers use the same `BoqAllocationEngine::allocate(...)` arguments,
CLI flags, and CSV template filenames. The returned hierarchy now preserves
the validated row-level allocation used by the HTML review: template groups
contain every used allocation item, and each allocation item contains its line
...
```json
{
  "metadata": {
    ...
    "unmapped_bills": 8,
    "total_records": 1759,
    "allocated_records": 1748,
    "unallocated_records": 11,
    "packages_used": 33,
    "work_items_used": 112,
      ...
      "children": [
        {
          ...
          "source_evidence": [
            "Item 42 | Bill 1: Earthworks | Section 1\nExcavate to reduce levels\nQuantity: 10 m3"
          ],
          "children": []
        }
      ]
