...
class AiProviderService
{
    ...
    public function classifyRecords(string $profile, string $dictionaryVersion, array $records): array
    {
        ...
        if ($estimatedMaxCost > (float)($this->config['cost_cap_usd'] ?? .15)) throw new Exception('AI request exceeds the configured AITOOLV3 cost ceiling.');
        $response = $this->httpPost('https://api.openai.com/v1/responses', [
            'model'=>$this->modelCatalog[$this->modelKey]['name'] ?? $this->modelKey,
            'reasoning'=>['effort'=>'low'],'store'=>false,
            'prompt_cache_key'=>hash('sha256', $profile . ':' . $dictionaryVersion),
            'max_output_tokens'=>$maxOutputTokens,'input'=>$input,
